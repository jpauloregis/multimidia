      ...
      for (int x = 0; x < 8; x++) {
        ...
        for (double v = 0; v < 8; v++) {
          ...
          for (double u = 0; u < 8; u++)
            innerloop += (DCT.C((int) u) / (double) 2) * matrix[(int) v][(int) u]
                * Math.cos((2 * x + 1) * u * Math.PI / (double) 16) * Math.cos((2 * y + 1) * v * Math.PI / (double) 16);
  ...
  public static float[][] slow_fdct(float[][] value) {
    ...
    for (int u = 0; u < 8; u++) {
      for (int v = 0; v < 8; v++) {
        ...
        for (int x = 0; x < 8; x++) {
          for (int y = 0; y < 8; y++) {
            innerval += value[y][x] * Math.cos(((2 * x + 1) * u * Math.PI) / 16)
